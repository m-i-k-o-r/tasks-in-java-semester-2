...
public class BinaryTreeAlgorithms {
    ...
    public static <T> int[] largeLevel(BinaryTree.TreeNode<T> treeNode, int[] list, Visitor<T> visitor) {
        // данный класс нужен только для того, чтобы "спрятать" его метод (c 3-мя параметрами)

        class Inner {
            void largeLevel(BinaryTree.TreeNode<T> node,int[] list, Visitor<T> visitor, int level) {
                if (node == null) {
                    return;
                }
                ...
                largeLevel(node.getLeft(),list, visitor, level + 1);
                largeLevel(node.getRight(),list, visitor, level + 1);
            }
        }
        // класс приходится создавать, т.к. статические методы в таких класс не поддерживаются
        new Inner().largeLevel(treeNode, list, visitor, 0);
        return list;
    }
